...
namespace WaterCloud.Web.Controllers
{
    ...
    public class ClientsDataController : Controller
    {
        ...
        private async Task<object> GetPrintTemplates()
        {
            var itemdata = await _ptService.GetList();
            Dictionary<string, string> dictionaryItem = new Dictionary<string, string>();
            foreach (var item in itemdata.Where(a => a.F_EnabledMark == true).ToList())
            {
                dictionaryItem.Add(item.F_PrintTemplateName, item.F_PrintTemplatePath);
            }

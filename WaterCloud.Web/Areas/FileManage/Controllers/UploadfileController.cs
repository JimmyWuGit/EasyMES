...
namespace WaterCloud.Web.Areas.FileManage.Controllers
{
    ...
    public class UploadfileController :  ControllerBase
    {
        ...
        [IgnoreAntiforgeryToken]
        public async Task<ActionResult> Upload(string fileby,int filetype =1)
        {
            try
            {
                //1图片，2excel,3fastreport模板,4json
                int[] filetypes = { 1, 2,3,4 };
                ...
                foreach (var file in files)
                {
                    ...
                    var isFrx = FileHelper.IsFrx(fileName);
                    if (filetype == 3 && !isFrx)
                    {
                        throw new Exception("请上传打印模板");
                    }
                    var isReport = FileHelper.IsReport(fileName);
                    if (filetype == 4 && !isReport)
                    {
                        throw new Exception("请上传报表模板");
                    }
                    if (ispic)
                    {
                        filetype = 1;
                    ...
                    if (ispic)
                    {
                        ...
                        {
                            FileHelper.DeleteFile(fileFullName);

...
namespace WaterCloud.Web.Areas.SystemOrganize.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitUserForm(string F_Account,string F_RealName, bool F_Gender,DateTime F_Birthday,string F_MobilePhone,string F_Email,string F_Description,string F_WebSocketPort)
        {
            try
            {
                ...
                userEntity.F_WebSocketPort = F_WebSocketPort;
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> DingSelfCancle()
        {
            try
            {
                await _service.DingCancle(_service.currentuser.UserId);
                return await Success("操作成功。", "", _service.currentuser.UserId, DbLogType.Update);
            }
            catch (Exception ex)
            {
                return await Error(ex.Message, "", _service.currentuser.UserId, DbLogType.Update);
            }
        }
        /// <summary>
        /// 钉钉清除
        ...
        public async Task<ActionResult> DingCancle(string keyValue)
        {
            try
            {
                await _service.DingCancle(keyValue);
                return await Success("操作成功。", "", keyValue, DbLogType.Update);
            }
            catch (Exception ex)
            {
                return await Error(ex.Message, "", keyValue, DbLogType.Update);
            }
        }
    }
}

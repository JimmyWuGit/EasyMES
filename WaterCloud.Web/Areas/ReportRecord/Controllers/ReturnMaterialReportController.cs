...
namespace WaterCloud.Web.Areas.ReportRecord.Controllers
{
    ...
    public class ReturnMaterialReportController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public async Task<ActionResult> GetGridJson(SoulPage<ReturnMaterialEntity> pagination, string keyword, int timetype = 2)
        {
            ...
            var data = await _service.GetReturnUseInfoList(pagination, timetype, keyword);
        ...
        {
            await _service.RemoveReturnUseRecord(keepTime);
            return Success("清空成功。");
        }
    }
}

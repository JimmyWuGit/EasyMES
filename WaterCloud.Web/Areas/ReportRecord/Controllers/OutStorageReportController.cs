...
namespace WaterCloud.Web.Areas.ReportRecord.Controllers
{
    ...
    public class OutStorageReportController : BaseController
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public async Task<ActionResult> GetGridJson(SoulPage<OutStorageInfoEntity> pagination, string keyword, int timetype = 2)
        {
            ...
            var data = await _service.GetOutStorageInfoList(pagination, timetype, keyword);
        ...
        {
            await _service.RemoveOutStorageRecord(keepTime);
            return Success("清空成功。");
        }
    }
}

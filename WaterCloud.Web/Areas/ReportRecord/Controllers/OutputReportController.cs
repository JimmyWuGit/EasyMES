...
namespace WaterCloud.Web.Areas.ReportRecord.Controllers
{
    ...
    public class OutputReportController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public async Task<ActionResult> GetGridJson(SoulPage<OutPutInfoEntity> pagination, string keyword, int timetype = 2)
        {
            ...
            var data = await _service.GetOutputInfoList(pagination, timetype, keyword);
        ...
        {
            await _service.RemoveOutputRecord(keepTime);
            return Success("清空成功。");
        }
    }
}

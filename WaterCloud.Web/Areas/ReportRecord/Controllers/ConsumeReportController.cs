...
namespace WaterCloud.Web.Areas.ReportRecord.Controllers
{
    ...
    public class ConsumeReportController : BaseController
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public async Task<ActionResult> GetGridJson(SoulPage<ConsumeInfoEntity> pagination, string keyword, int timetype = 2)
        {
            ...
            var data = await _service.GetConsumeInfoList(pagination, timetype, keyword);
        ...
        {
            await _service.RemoveConsumeRecord(keepTime);
            return Success("清空成功。");
        }
    }
}

...
namespace WaterCloud.Web.Areas.PlanManage.Controllers
{
    ...
    public class OutStoragePlanController :  ControllerBase
    {
        ...
        [HandlerAjaxOnly]
        [IgnoreAntiforgeryToken]
        public async Task<ActionResult> GetListJson(SoulPage<OutStorageEntity> pagination, string keyValue, string materialId)
        {
            if (string.IsNullOrEmpty(pagination.field))
            {
                pagination.field = "F_OutStorageState asc,F_PlanTime asc";
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitForm(OutStorageEntity entity, string id)
        {
            try
            {
                entity.F_Id = id;

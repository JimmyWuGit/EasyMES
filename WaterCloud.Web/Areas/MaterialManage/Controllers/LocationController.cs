...
namespace WaterCloud.Web.Areas.MaterialManage.Controllers
{
    ...
    public class LocationController :  ControllerBase
    {
        ...
        public async Task<ActionResult> GetGridJson(string itemId,SoulPage<LocationEntity> pagination, string keyword)
        {
            if (string.IsNullOrEmpty(pagination.field))
            {
                pagination.field = "F_LocationState,F_MaterialName,F_SortCode,F_LocationCode";
                pagination.order = "asc";
            }
            var data = await _service.GetLookList(itemId,pagination, keyword);
        ...
        {
            var data = await _service.GetList(keyword);
            data = data.Where(a => a.F_LocationState == false).ToList();
            return Content(data.ToJson());
        }
        [HttpGet]
        [HandlerAjaxOnly]
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> SubmitForm(LocationEntity entity, string keyValue)
        {
            try
            {
        ...
        {
            var filterList = JsonConvert.DeserializeObject<List<LocationEntity>>(listData);
        ...
        [HttpPost]
        [HandlerAjaxOnly]
        public async Task<ActionResult> BandingForm(string id, string keyValue)
        {
            try
            {
                await _service.BandingForm(id, keyValue);
                return await Success("操作成功。", "", keyValue);
            }
            catch (Exception ex)
            {

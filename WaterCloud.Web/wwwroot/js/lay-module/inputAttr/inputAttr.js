...
layui.define(["layer", "jquery"], function (exports) {
    ...
    var filePath = layui.cache.modules.inputAttr
        .substr(0, layui.cache.modules.inputAttr.lastIndexOf('/'));
    // 引入tablePlug.css
    layui.link(filePath + '/inputAttr.css');
    ...
    function init(opts) {
        ...
        // 匹配表单节点
        var $doms = $root.find("input,textarea");
        ...
        $doms.each(function () {
            ...
            if (hasClearable && tagName === "input") {
                ...
                $el.on("input", toggle).on("focus", toggle).on("blur", function (e) {
                    $i.css({opacity: 0})
                });

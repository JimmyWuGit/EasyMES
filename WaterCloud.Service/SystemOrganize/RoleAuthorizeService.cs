...
namespace WaterCloud.Service.SystemOrganize
{
    public class RoleAuthorizeService : DataFilterService<RoleAuthorizeEntity>, IDenpendency
    {
        ...
        public async Task<List<ModuleEntity>> GetMenuBoardList()
        {
            var data = new List<ModuleEntity>();
            data = await moduleApp.GetList();
            var parent =await uniwork.FindEntity<ModuleEntity>(a => a.F_EnCode == "RunMonitor");
            data = data.Where(a => a.F_IsMenu == true && a.F_EnabledMark == true && a.F_ParentId == parent.F_Id&&a.F_EnCode!= "BoardAccount").ToList();
            return data.OrderBy(t => t.F_SortCode).ToList();
        }

		public async Task<List<ModuleButtonEntity>> GetDingMenuList(string roleId)
		{
            var data = new List<ModuleButtonEntity>();
            if (currentuser.IsSystem)
            {
                data = await moduleButtonApp.GetDingMenuList();
            }
            else
            {
                var buttondata = await moduleButtonApp.GetDingMenuList();
                var role = await roleApp.GetForm(roleId);
                if (role == null || role.F_EnabledMark == false)
                {
                    return data;

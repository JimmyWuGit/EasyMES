    ...
    public class LogService : DataFilterService<LogEntity>, IDenpendency
    {
        ...
        public async Task<List<LogEntity>> GetList(Pagination pagination, int timetype, string keyword="")
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(keyword))
                {
                    result = result.Where(t => t.F_Date >= startTime && t.F_Date <= endTime).Where(u => u.F_Account.Contains(keyword) || u.F_Description.Contains(keyword) || u.F_ModuleName.Contains(keyword)).ToList();
                }
                else
                {
